...
The color story is starkly binary. Product sections alternate between pure black (`#0A0A0A`) backgrounds with white text and light gray (`#F5F7FA`) backgrounds with near-black text (`#111827`). This creates a cinematic pacing — dark sections feel immersive and premium, light sections feel open and informational. The only chromatic accent is Apple Blue (`#38040E`), reserved exclusively for interactive elements: links, buttons, and focus states. This singular accent color in a sea of neutrals gives every clickable element unmistakable visibility.

**Key Characteristics:**

- SF Pro Display/Text with optical sizing — letterforms adapt automatically to size context
- Binary light/dark section rhythm: black (`#0A0A0A`) alternating with light gray (`#F5F7FA`)
- Single accent color: Apple Blue (`#38040E`) reserved exclusively for interactive elements
...
- **Pure Black** (`#0A0A0A`): Hero section backgrounds, immersive product showcases. The darkest canvas for the brightest products.
- **Light Gray** (`#F5F7FA`): Alternate section backgrounds, informational areas. Not white — the slight blue-gray tint prevents sterility.
- **Near Black** (`#111827`): Primary text on light backgrounds, dark button fills. Slightly warmer than pure black for comfortable reading.

### Interactive

- **Apple Blue** (`#38040E`): `--sk-focus-color`, primary CTA backgrounds, focus rings. The ONLY chromatic color in the interface.
- **Link Blue** (`#640D14`): `--sk-body-link-color`, inline text links. Slightly darker than Apple Blue for text-level readability.
- **Bright Blue** (`#B53B33`): Links on dark backgrounds. Higher luminance for contrast on black sections.

### Text

- **White** (`#ffffff`): Text on dark backgrounds, button text on blue/dark CTAs.
- **Near Black** (`#111827`): Primary body text on light backgrounds.
...
- **Dark Surface 1** (`#1C1C1E`): Card backgrounds in dark sections.
- **Dark Surface 2** (`#1B1B1D`): Subtle surface variation in dark contexts.
- **Dark Surface 3** (`#1E1E20`): Elevated cards on dark backgrounds.
- **Dark Surface 4** (`#202023`): Highest dark surface elevation.
- **Dark Surface 5** (`#191919`): Deepest dark surface tone.

### Button States

- **Button Active** (`#F3F4F6`): Active/pressed state for light buttons.
- **Button Default Light** (`#F9FAFB`): Search/filter button backgrounds.
- **Overlay** (`rgba(203, 194, 185, 0.64)`): Media control scrims, overlays.
- **White 32%** (`rgba(255, 255, 255, 0.32)`): Hover state on dark modal close buttons.

### Shadows

- **Card Shadow** (`rgba(56, 4, 14, 0.12) 3px 5px 30px 0px`): Soft, diffused elevation for product cards. Offset and wide blur create a natural, photographic shadow.
...
- Active: `#F3F4F6` background shift
- Focus: `2px solid var(--sk-focus-color, #38040E)` outline
...
- Text: `#640D14` (light bg) or `#B53B33` (dark bg)
...
- Background: `rgba(203, 194, 185, 0.64)`
- Text: `rgba(0, 0, 0, 0.48)`
- Radius: 50% (circular)
- Active: scale(0.9), background shifts
- Focus: `2px solid var(--sk-focus-color, #38040E)` outline, white bg, black text
- Use: Play/pause, carousel arrows

### Cards & Containers

- Background: `#F5F7FA` (light) or `#1C1C1E`-`#202023` (dark)
- Border: none (borders are rare in Apple's system)
- Radius: 5px-8px
- Shadow: `rgba(56, 4, 14, 0.12) 3px 5px 30px 0px` for elevated product cards
...
- Full-viewport-width section with solid background (black or `#F5F7FA`)
...
- **Vertical rhythm through color blocks**: Rather than using spacing alone to separate sections, Apple uses alternating background colors (black, `#F5F7FA`, white). Each color change signals a new "scene."
...
| Subtle Lift (Level 1) | `rgba(56, 4, 14, 0.12) 3px 5px 30px 0px`                          | Product cards, floating elements           |
| Media Control         | `rgba(203, 194, 185, 0.64)` background with scale transforms      | Play/pause buttons, carousel controls      |
| Focus (Accessibility) | `2px solid #38040E` outline                                       | Keyboard focus on all interactive elements |
...
- Use Apple Blue (`#38040E`) ONLY for interactive elements — it must be the singular accent
- Alternate between black and light gray (`#F5F7FA`) section backgrounds for cinematic rhythm
...
- "Create an alternating section layout: first section black bg with white text and centered product image, second section #F5F7FA bg with #111827 text. Each section near full-viewport height with 56px headline and two pill CTAs below."
- "Design a 'Learn more' link: text #640D14 on light bg or #B53B33 on dark bg, 14px SF Pro Text, underline on hover. After the text, include a right-arrow chevron character (>). Wrap in a container with 980px border-radius for pill shape when used as a standalone CTA."

### Iteration Guide

1. Every interactive element gets Apple Blue (`#38040E`) — no other accent colors
2. Section backgrounds alternate: black for immersive moments, `#F5F7FA` for informational moments
